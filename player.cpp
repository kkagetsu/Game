...
VOID Player :: GraphicLoad() {
	...
	grHandle[TACHI1_E] = LoadGraph("picture/player1/player_tachie1.png");//たち絵１をロードする 
	grHandle[TACHI2_E] = LoadGraph("picture/player1/player_tachie2.png");//たち絵２
	grHandle[WAKU1__E] = LoadGraph("picture/player1/player_waku1.png")  ;//playerui用キャラアイコン1
	grHandle[WAKU2__E] = LoadGraph("picture/player1/player_waku2.png")  ;//playerui用キャラアイコン2
	grHandle[PIXEL__E] = LoadGraph("picture/player1/player_pixel.png")  ;//ピクセル画像(バトルフィールド用)
																		 
	grHandleUI[MOVE_E]      = LoadGraph("picture/player1_ui/move.png");	 //playerui用move
    grHandleUI[ATTACK_E]	= LoadGraph("picture/player1_ui/attack.png");//playerui用attack
    grHandleUI[ITEAM_E]		= LoadGraph("picture/player1_ui/iteam.png") ;//playerui用item
    grHandleUI[WAIT_E]		= LoadGraph("picture/player1_ui/wait.png")  ;//playerui用wait
    grHandleUI[SPSKILL_E]	= LoadGraph("picture/player1_ui/wolf.png")  ;//playerui用spskill
    grHandleUI[LEARNSKILL_E]= LoadGraph("picture/player1_ui/book.png")  ;//playerui用learnskill
    grHandleUI[SKILL_1_E]   = LoadGraph("picture/player1_ui/skill1.png");//playerui用skill1
...
VOID Player::PlayerAbleMove() {
	...
	for (int dy = -move; dy <= move; dy++) {     // Y軸が　-3 から　+３まで　1づつ　縦プラス  
		for (int dx = -move; dx <= move; dx++) { // X軸が　-3 から　+３まで　1づつ　横プラス
			//                ・・・・・・・  
			//				  ・・・・・・・
			//				  ・・・・・・・
			//				  ・・・ｐ・・・
			...
			if (abs(dx) + abs(dy) <= move) { // ダイヤモンド形の範囲内かをチェック
				...
				//　　　　　　　　・・・
				//　　　　　　　　　・
				//if分によって　座標dx+dyの値が    <= moveの座標を保留し    >  座標全部pass
				int x = posx + dx;  //現在playerの座標ｘ　＋　保留されたｄｘ　
				int y = posy + dy;  //現在playerの座標Y　 ＋  保留されたｄY　
				...
				if (x >= 0 && x < GRID__WIDTH && y >= 0 && y < GRID_HEIGHT) {
					...
					// 岩、川、壁のマスをスキップ
					E_LAYOUT_T layout = g_layout[y][x]; // x,y的坐标对应地图上的xy地形配列
					//layoutの中身が岩、川、壁ではない場合
					if (layout != LAYOUT_STONE && layout != LAYOUT_DRIVE && layout != LAYOUT_WALL) {
						...
						DrawBox(drawMASUX, drawMASUY1, drawMASUX2, drawMASUY2, 0x0000ff, TRUE); // 蒼色で描画
						// 描画ブレンドモードを元に戻す
